...
export class Character {
    ...
    handleStateTransitions() {
        ...
        let targetState = characterStates.STANDING;
    ...
    // Recibir daño
    takeDamage(damage, knockback, hitstun) {
        this.health -= damage;
        if (this.health < 0) this.health = 0;
        
        // Aplicar knockback
        this.velocity.speedx = knockback.x;
        this.velocity.speedy = knockback.y;
    ...
    update(secondspassed, opponent) {
        ...
        if (!this.animationFinished || !isOneTimeAnimation) {
            ...
            if (this.animationTimer >= currentAnimationTimeLimit) { 
                ...
                if (isOneTimeAnimation && this.animationFrame >= totalFrames) {
                    ...
                    // Si terminó un ataque, volver a standing
        ...
        if (opponent && isAttackState(this.currentState) && !this.hasHitThisAttack) {
            ...
            if (this.checkBoxCollision(myHitbox, opponentHurtbox) && !opponent.isInHitstun) {
                opponent.takeDamage(myHitbox.damage, myHitbox.knockback, myHitbox.hitstun);
    ...
    draw(ctx) {
        ...
        const drawWidth = this.size.width;
        ...
        if (this.spriteLoaded) {
            ...
            const rectY = this.position.y - drawHeight;
            ctx.fillStyle = '#ff0000';
            ctx.fillRect(rectX, rectY, this.size.width, drawHeight);
        }
        ...
        ctx.fillText(`${this.currentState} | HP: ${this.health}`, this.position.x, this.position.y - drawHeight - 10);
    }
